...
def process(image, processor_id):
    ...
    if processor_id in processor_cache:
        ...
        processor_cache[processor_id] = processor # 使用controlnet aux的processor处理得到控制图
    for img in image:
...
def softedge_prepreocess(image, proc = "pidsafe", **kwargs): # 输入一系列视频帧
...
def depth_prepreocess(image, **kwargs):
    ...
    depth_estimator = transformers.pipeline('depth-estimation',save_path='/nfs/users/zhangxinyu/VidToMe/lllyasviel/control_v11f1p_sd15_depth')
